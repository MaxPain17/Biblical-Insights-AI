...
import type { ProverbialConnectionItem } from '../types';

interface ProverbialConnectionsProps {
  data: ProverbialConnectionItem[];
  onReferenceSelect: (reference: string) => void;
}

export const ProverbialConnections: React.FC<ProverbialConnectionsProps> = ({ data, onReferenceSelect }) => {
  if (!data || data.length === 0) {
    return <p className="text-text-muted">No specific proverbial connections were found for this verse.</p>;
  }
    ...
    <div className="space-y-4">
      {data.map((item, index) => (
        ...
        >
          ...
          <div>
            <span className="text-xs font-medium text-text-muted uppercase tracking-wider">Connection</span>
            <p className="text-text-main mt-1">{item.connection}</p>
          </div>
        </div>
      ))}
    </div>
